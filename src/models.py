...
def get_model(image_encoder_model, text_decoder_model, saved_model_path=None):
    if saved_model_path is not None:
        ...
        if image_encoder_model == 'timber/resnet' and text_decoder_model == 'timber/bert-decoder':
            ...
            encoder_config = AutoConfig.from_pretrained(image_encoder_model)

            class TimberResNet(ResNetModel):
                def __init__(self, config):
                    super(TimberResNet, self).__init__(config)
                    self.config.hidden_size = 2048*7*7 # this is a 224x224 image run through the model and flattened
            ...
            decoder = AutoModelForCausalLM.from_config(AutoConfig.from_pretrained(text_decoder_model))
